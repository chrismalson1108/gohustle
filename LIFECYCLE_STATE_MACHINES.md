...
- `credit_earnings(p_payment_id)` (latest: `20260906040000_credit_earnings_honours_the_recorded_clawback.sql`): a single conditional UPDATE flips `earnings_credited` false→true **only if** `status='captured'` and amount>0, then increments `profiles.earnings_today/week/total` in the same transaction. Called by **both** the capture edge fn (`stripe-capture-payment:457`) and the `payment_intent.succeeded` webhook (`stripe-webhook:329`) — **idempotent, exactly-once** even under concurrent capture+webhook. EXECUTE revoked from public/anon/authenticated. ✅
- ⚠️ **It credits `earner_amount_cents` NET of `earner_refunded_cents`, not the gross.** `record_refund` records the earner's share of a reversal unconditionally, but `debit_earnings` refuses to debit a payment whose `earnings_credited` is false (`20260804020000:44-46`) — deliberately, since debiting for money never credited is worse. So a refund on a capture whose credit failed (the `ctl_earner_credit_missing` state) leaves a clawback *recorded and unapplied*, and crediting the gross afterwards overstated the earner by exactly what Stripe pulled back out of their Connect account. The claim UPDATE now returns both figures under one row lock and withholds the difference. A lost chargeback records 0 and so still credits in full (`20260813160000` — the platform absorbs it). ✅

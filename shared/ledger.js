// ─────────────────────────────────────────────────────────────────────────────
// The money ledger — the pure half. Every transaction a user has been part of,
// restated for whichever side of it they are on.
//
// This lived in src/lib/payments.js and existed only there, which is why the WEBSITE
// had no ledger at all: no receipts, no refunds, no pinned fee rate, no CSV, no bank
// deposits — while Hustlr AI (one prompt, served to both clients) told every user to
// open "You → Payments & payouts → Transactions". CLAUDE.md's rule for exactly this
// case is that shared/ is the single source and logic goes here rather than into one
// client, and money arithmetic is the last place to keep two copies: the fee is PINNED
// per booking, the refund split differs by side, and a partial capture deliberately
// leaves amount_cents at the full authorization. A second hand-ported copy of that
// would disagree with the first within a release.
//
// Everything below is pure. The two Supabase reads stay in each client's own module
// (src/lib/payments.js, web/lib/payments.ts) because the client differs; the maths
// they feed does not. __tests__/ledger.test.js exercises it through the mobile
// re-export, so it covers both.
//
// ── WHAT THE FIGURES MEAN ───────────────────────────────────────────────────
//
// Amounts come from the payment ROW, never re-derived from the current rate card. A
// booking's fee was pinned when it was made, and showing a past transaction at
// today's rate misstates what the person actually paid or received.
//
// ── BANK ARRIVAL: known, and still not over-claimed ─────────────────────────
//
// Capture is a destination charge, so funds land in the earner's Stripe Connect
// account immediately and Stripe pays out on that account's own schedule.
// stripe_payouts stores those payout.* events, so arrival dates are real Stripe data.
// What is still NOT claimed is which gig sits in which deposit — Stripe batches many
// transfers into one payout.
// ─────────────────────────────────────────────────────────────────────────────
import { DEFAULT_FEE_BPS, feeLabel, platformFeeCents, effectiveFeeLabel } from './pricing.js';
 ...
 */
export function toEntry(row, side, jobsById, bookingsById) {
...
// `new Date(y, 0, 1)`, byMonth() keys on getFullYear()/getMonth(), and both screens
// render each row with toLocaleDateString() — so formatting the export with

...
const PLACES_MOBILE = `- Places people ask about, so you can point them straight there:
  ...
  · A problem with finished work — either side of it. When a poster verifies a gig they can tick "There was a problem" and propose paying less, with a reason and photos. That no longer pays out on its own: the money stays held, the earner is alerted, and My Jobs shows them a banner on that gig with 48 hours to accept the amount or say what actually happened and attach their own photos. Accepting — or saying nothing — settles it at the proposed amount; disputing it means nobody is paid until a person at GoHustlr has read both sides. If an earner says the poster is refusing to pay, is describing the wrong job, or "reported something that isn't mine", send them to that banner: it is the whole answer, and the clock is real.

...
- **`guard_bookings_write`** (current `20260702030000_guard_pins_and_slot_delete_policies.sql:15-116`), the transition/column arbiter:
  ...
  - **Poster branch, additionally pinned when the poster is suspended or the pair is blocked** (`20260906055000_blocks_and_suspension_reach_reviews_and_amendments.sql`): `amendment_status, amendment_note, review_text, earner_rating, payment_method` — every poster-authored column that renders in the earner's My Jobs, the amendment note with a push behind it. **Lifecycle is untouched**: decline, cancel, complete and verify still work, because suspension must never strand an escrow hold or withhold money already earned. Pinned rather than raised so the block stays silent (`20260710030000`), and the two `private.*` helpers are consulted only when one of those columns actually moves.
...
- `reviews_insert_auth` (current `20260906055000_blocks_and_suspension_reach_reviews_and_amendments.sql`): requires `auth.uid()=reviewer_id` **AND** a `verified` booking **on the same `job_id`** where the reviewer/reviewed pair matches the direction, **and `role` matches** (`poster→earner ⇒ role='earner'`; `earner→poster ⇒ role='poster'`). This closed the earlier "one verified booking authorizes unlimited/arbitrary-job reviews" flaw (`20260624210000_review4_db_fixes.sql`). Since `20260906055000` it **also requires `not private.is_blocked_pair(earner, poster)` and `not private.is_suspended(auth.uid())`** — a review is public and, per RUNBOOK_SAFETY §2.3, cannot be redacted, so a party the platform has decided must not reach the other may not publish one. Blocks are **bidirectional** (a one-way rule would make blocking a race to silence the other's review); suspension checks the **actor only**, matching `messages_insert`.
...
- **Admins cannot** rewrite/delete audit history, or instantly kill an already-issued access JWT. (Two former entries here are now FALSE as of 2026-08-04: `admin_users` IS managed from `/team`, and refunds/hold-releases DO run from `/bookings/[id]` via the `admin-payment-action` edge function.) Historical wording follows: admins cannot manage `admin_users` from the console (DB-only), refund/capture payments from the console (Stripe-side only), or instantly kill an already-issued access JWT (refresh sessions are revoked; existing access tokens live up to ~1h — Supabase-inherent).

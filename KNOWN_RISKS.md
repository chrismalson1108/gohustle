...
| T-1 | Low (cost) | **The platform pays Stripe's 2.9% + 30¢ on every tip and takes nothing.** `stripe-tip` creates the PaymentIntent with `transfer_data.destination` and NO `application_fee_amount`, so 100% of a tip reaches the earner and processing comes out of the platform's pocket. | **DECIDED 2026-08-17: keep it, deliberately.** Passing tips through whole is the industry norm — DoorDash, Instacart, Uber and Lyft all route 100% to the worker, and every attempt to do otherwise has become a public scandal rather than a revenue line. On a platform whose whole pitch is that workers keep their earnings, skimming a tip is the single most expensive dollar it could take. The cost is bounded and small: 2.9%+30¢ of tip volume, an order of magnitude under the fee revenue on the gigs the tips attach to. **Revisit only if tip volume becomes large relative to gig volume**, which would itself mean the rail is being used for something other than tipping. | `supabase/functions/stripe-tip/index.ts:143-159`; caps + reservation in `20260814090000` |

**Impact:** these are conscious deferrals. T-1 is a priced marketing decision rather than a gap — the number is known and accepted. D-2's "High" is advisory only (not in the runtime bundle); H-4 becomes real only if the app ever adopts cookie/ambient auth; Q-1 is code-quality; F-5 recurs from §1.4.
...
16. **Sign in with Apple tokens are not revoked on account deletion** (App Store 5.1.1(v), via Apple's *Offering account deletion in your app*). `delete-account` removes storage objects, releases escrow holds and calls `auth.admin.deleteUser`, but never calls Apple's `POST https://appleid.apple.com/auth/revoke`.

    **Why it is not implemented:** the native flow (`AuthContext.signInWithApple`) uses `signInWithIdToken`, which never captures Apple's `authorizationCode`, so **no Apple refresh token is stored anywhere** — there is nothing to revoke with today. Implementing it requires all of: (a) capturing `credential.authorizationCode` at sign-in; (b) exchanging it with Apple within its ~5-minute lifetime, which needs a client-secret JWT signed with the Apple **.p8 private key** plus `APPLE_TEAM_ID` / `APPLE_KEY_ID` / `APPLE_CLIENT_ID`; (c) storing the resulting refresh token; (d) calling `/auth/revoke` from `delete-account`. None of those Apple credentials are configured, and the change touches a working critical auth path that cannot be exercised without them — a half-built version risks breaking Apple sign-in, which is far worse than the gap.

    **Severity:** enforcement is inconsistent; the 2026-07-22 App Store readiness review rated it *likely-rejection*, not a hard blocker. Do it deliberately with the Apple key to hand, not as a rushed pre-submission patch.

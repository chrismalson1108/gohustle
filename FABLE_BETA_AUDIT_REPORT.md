...
> **Remediation update (this branch).** The High-severity code blockers **H1–H8** have been implemented as the smallest safe changes, each with tests (`npm test` green: 16 suites / 110 tests). See **§4.1.1** for the per-item status and the exact migrations/functions/edits. This does **not** by itself flip the verdict: enforcement of the DB-layer fixes remains **live-conditional** on the new migrations being applied (`supabase db push`) and a few edge-function secrets/GUCs being set, and the config-only / later-phase High items (**H9/H10/H11/H12**) are intentionally out of this pass. The launch decision's evidence checklist still governs GO.

...
| `PostJobScreen` | Post a new gig — LocationPicker + DateTimePicker + custom "Other" category chip. **At least one time slot is required** (booking flows around slot selection; the old "Flexible — Contact to Schedule" fallback slot is gone). Same rule on save in EditJob. Nested in GigsStack. |
